  ...
  $.fn.mediumInsert.images = {
    ...
    */
    ...
    uploadCompleted: function (jqxhr) {
      ...
      $progress.html(100);
      $progress.before('<span class="mediumInsert-images" guid="'+jqxhr.responseText+'"><img src=/loadimage?guid='+ jqxhr.responseText +' draggable="true" alt=""></span>');
    ...
    uploadFiles: function ($placeholder, files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (acceptedTypes[file.type] === true) {
          ...
          $.ajax({
            type: "post",
            url: "/register/uploadimage?guid="+$('#guid').val(),
            xhr: xhr,
            cache: false,
            contentType: false,
    ...
    */
    ...
    setImageEvents: function () {
      ...
      this.$el.on('click', '.mediumInsert-imageRemove', function () {
        ...
        $(this).parent().remove();
        var imageGuid = $(this).parent().attr('guid');
          $.ajax({
              type: "post",
              url: "/register/removeimage?guid="+$('#guid').val()+"&imageguid="+imageGuid,
